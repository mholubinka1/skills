...
3. A job matching both 1 and 2 is a **Reachable self-hosted job** finding — unless it already
   ...
   rule (`gh api repos/{owner}/{repo}/environments/{that name} --jq '.protection_rules'`, same
   check the "gate behind approval" fix itself uses): that job is already gated, so don't
   flag it again. This matters because that fix's own naming scheme is deterministic
   (`<job-name>-manual-approval`) specifically so this re-check recognizes it on a later
   audit — without this exclusion, a fully-applied fix would still show up as an unresolved
   Finding on every subsequent run.
