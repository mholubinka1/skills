...
| Restrict trigger — gate behind approval | 0. Derive `<env-name>` (not the same placeholder as `jobs.<name>` in *Trigger crossing* above — this one names the GitHub Environment, a separate resource, and — verified against GitHub's workflow syntax docs — a job can only ever reference one, as a plain string or a `{name, url}` object, never a list, which is exactly why steps 0 and 5 below go to this trouble): if the reachable job **already has an `environment:` key**, first check whether any *other* job — in this workflow file or any other under `.github/workflows/` — references that same environment name (read the files directly and reason, per this skill's convention; no parsing script). If so, it's a **shared environment** — do not reuse it: configuring `required_reviewers` on it would silently add an approval gate to every other job that references it too, an unrequested blast radius this skill doesn't take on implicitly. Report "can't gate `<env-name>` — already shared with another job; add a dedicated `environment:` to this job manually" and stop; don't proceed to step 1. If no other job references it, reuse that exact value as `<env-name>` — protect the environment this job already references instead of inventing a second one, so step 5 never needs to add or replace a key. Only if the job has no `environment:` key yet, derive one deterministically from the job's own name (the same `<name>` from `jobs.<name>` there), so a repeat run recognizes the same environment instead of creating a new one each time: `<env-name>` is `<name>-manual-approval`, e.g. a job named `deploy` gets the environment `deploy-manual-approval`. 1. Check whether `<env-name>` already requires manual approval specifically: `gh api repos/{owner}/{repo}/environments/<env-name> --jq '.protection_rules'` — look for an entry whose `type` is `required_reviewers` (verified live by creating a real test environment with a configured reviewer: `protection_rules[].type` is exactly `"required_reviewers"`, distinct from `wait_timer` or `branch_policy`, neither of which involves a human). If one is already present, leave it alone. A 404, an empty array, or an array with only `wait_timer`/`branch_policy` entries all mean it isn't gated behind approval yet — proceeding is safe either way: this endpoint merges rather than replaces, so a step-4 PUT that only sends `reviewers`/`prevent_self_review` leaves a pre-existing `wait_timer` or `deployment_branch_policy` untouched (verified live: created an environment with only a `wait_timer` configured, PUT again with only `reviewers`, and the `wait_timer` rule was still present in the response alongside the new one). 2. Fetch the admin-collaborator list once, already capped at GitHub's 6-reviewer limit (verified against GitHub's REST docs), and reuse it for both this step and the next — never re-fetch: `ADMINS=$(gh api repos/{owner}/{repo}/collaborators --jq '(map(if .role_name == "admin" then {type:"User", id} else empty end))[0:6]')`. If `$ADMINS` is `[]` (no admin collaborators, a theoretical edge case), stop and report "can't gate `<env-name>` — no eligible reviewer found"; do **not** proceed to step 3, since an empty `reviewers` array creates zero protection rules (verified live) — the exact no-op this whole fix exists to avoid. 3. Otherwise build the request body from `$ADMINS`, with `prevent_self_review` set based on whether there's a *second* admin collaborator — `true` if so (without it, the PR author could rubber-stamp their own gated run if they're in the reviewer list, verified live that the field takes effect); `false` for exactly one admin. Note this "one admin" count is a different population than the Collaborators check's "solo-maintained" (which counts *every* collaborator, any role) — a repo with one admin plus non-admin collaborators still hits this `false` branch, since only admins can be environment reviewers here; only an admin count of exactly one forces the same deadlock-avoidance reasoning as the solo-maintained branch-protection row further below (the sole eligible reviewer must be able to approve their own run, or the gate deadlocks every run it protects, forever, with no one able to unblock it): `BODY=$(jq -n --argjson reviewers "$ADMINS" '{reviewers: $reviewers, prevent_self_review: ($reviewers[1] != null)}')`. 4. Send it as the real request body: `gh api --method PUT repos/{owner}/{repo}/environments/<env-name> --input - <<< "$BODY"` — `--input` is used here rather than `-f`/`-F` because `reviewers` is an array of objects, a shape `-f`/`-F`'s bracket syntax has no verified construction for (the same reason `checks` was left as future work below); `--input` sends the JSON body directly, so there's nothing to get wrong. 5. Only then add `environment: <env-name>` to the reachable job — but only if it didn't already have an `environment:` key in step 0 (in which case `<env-name>` came from that existing key, and it's already there; adding a second `environment:` key would be invalid YAML either way, but this makes it structurally impossible rather than something to remember to check for). | The only trigger-restricting fix for `pull_request`/`pull_request_target` — branch filtering can't exclude a bot there (see the row below). A reachable `push` is always handled by the row below instead, whichever of its three cases applies. **Step 5 alone is not the fix.** Referencing an environment name with no protection rules doesn't gate anything — GitHub auto-creates it with none (verified live against a real repo, and against GitHub's docs) and the job runs exactly as if `environment:` were never added. Steps 1–4 are what actually restrict the trigger; step 5 only wires the job to them. |
...
| Branch protection — multi-maintainer (2+ collaborators) | Same as above, but `-F 'required_pull_request_reviews[required_approving_review_count]=1'` instead of `=0` | Everything else identical to the solo-maintained payload. Always a flat `1` — this fix distinguishes only solo from non-solo, never scaling the count with collaborator count. `required_approving_review_count`'s valid range is 0–6 either way (verified against GitHub's REST docs); not relevant here since the value used is always `1`. |
