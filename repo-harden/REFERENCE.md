...
3. A job matching both 1 and 2 is a **Reachable self-hosted job** finding — unless it already
   ...
   (`<name>-manual-approval`, using the same `jobs.<name>` from rule 2 above) specifically so
   this re-check recognizes it on a later
