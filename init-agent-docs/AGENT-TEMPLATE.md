...
- Never use unanchored `sed`/regex find-replace to rename an identifier across files — a pattern like `_parse_window_date` will also match inside `test_parse_window_date_x`, silently eating the underscore boundary. Use word-boundary-anchored patterns (e.g. `\b`) or a proper multi-file rename that respects token boundaries instead.
