...
- Take ownership of a task from start to finish with minimal handholding. This includes clarifying ambiguity, scoping the work, implementing it, verifying it locally, and confirming it works in the target environment.
...
- Deliver consistently against the stated scope; your effort estimates must be broadly reliable.

---

## 2. Production Code Quality

- Write readable code above all else. Code must be self-documenting. Use comments only to explain complex functionality — not to restate what the code does.
- Always favour clarity over brevity. Maintainability is usually preferable to micro-efficiency.
...
- Name everything descriptively. Variables, methods, classes, and files must communicate intent without requiring the reader to open another file.
...
- Use logging levels (`info`, `debug`, `warning`, `error`) appropriately. Log messages must be meaningful.
- Always follow the existing patterns and conventions of the codebase. Introduce new patterns slowly, with good reason and explicit agreement.
...
- Any change to execution logic must be accompanied by tests. If code can be changed without tests failing, the current test coverage is insufficient.
...
- Write tests that verify behaviour, not implementation. Tests that test internal structure have no value.
...
- All tests must pass locally before any PR is raised. Do not raise a PR with a failing pipeline.
...
- Make commits small, logical, and with meaningful descriptive messages. Loose messages (`fix`, `wip`, `update`) are not acceptable on reviewed branches.
...
- Read acceptance criteria before starting. Clarify ambiguities upfront. Validate your implementation against acceptance criteria before raising a PR. Proactively write acceptance criteria when they are absent.
- You are the first line of behaviour verification — not QA. Test changes appropriately in the execution environment before requesting review.
