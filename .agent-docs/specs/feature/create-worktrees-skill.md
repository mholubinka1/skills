...
5. As a user, I want the worktree automatically removed once `/implement` confirms my PR has merged, so that I don't accumulate stale worktrees I have to clean up by hand.
...
- **Gitignore hygiene**: check for a `.claude` line in `.gitignore` (`grep -qxF '.claude' .gitignore`). If missing (including when `.gitignore` doesn't exist), append it and commit as a standalone commit (`chore: gitignore .claude/`) on whatever branch is currently checked out, before any worktree is created.
...
- **`/implement` integration** (`implement/WORKFLOW.md`):
  ...
  - All subsequent steps renumber (old Step 1–7 become Step 1–8, offset by the inserted Step 0... concretely: bootstrap agent docs becomes Step 1, grill Step 2, branch hygiene Step 3, write spec Step 4, create issues Step 5, BDD loop Step 6, code review Step 7, merge Step 8).
